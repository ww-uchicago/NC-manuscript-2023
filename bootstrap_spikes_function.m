function [SortedDSData filtData] = bootstrap_spikes_function(varargin)
...
null_ind = find(abs((directions-null_dir)) < eps('single')); %hack to find match
pref_ind = find(abs((directions-pref_dir)) < eps('single')); %hack to find match
