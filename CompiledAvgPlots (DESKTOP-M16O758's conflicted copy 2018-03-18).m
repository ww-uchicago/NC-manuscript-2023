...
%% main part
folder = 'C:\Users\Hector Acaron\Desktop\Wei Lab\Data\20180313';
...
for i = 1:length(cell_area)
    ...
    %         
            ...
            ptrace{1,t} =cellfun(@(x) reconstruct(x),data.signalNew,'uniformoutput',false);
            for i=1:nreps
            ...
            for c=1:min(stim_numCells)
                ...
                for b = 1:length(movieNums)
                    ...
                    if b == 1
                        ...
                        shadedErrorBar(linspace(0, length(dlist)*time(b), size(ptrace{1,b}{1,1},1)),y,err, {'-k','Linewidth',2})
                        ylabel('a.u.')
                        xlabel('Time (seconds)')
