...
folder= uigetdir('C:\Users\admin\Desktop\Wei Lab HD\recordings\New Rig\20160210');
...
[data.vlocs,data.pks]=peakdetect(data.vol,2,40000);
...
reps=reshape(1:40,8,5); %reps(:,rm_rep)=[]; reps=reshape(reps,size(reps,1)*size(reps,2),1); nreps=length(reps)/8;
% clip the dataset
...
movieNum = input('What is the number of this movie?\n');
...
for i=1:size(data.mamp,2)
    ...
    [ ll,dsi, osi, angle,idx ,nidx, o1dx, o2dx, os_angle]=vectorsum(data.mamp{i});
    title(['ROI' ' '  num2str(rois(i))]);
